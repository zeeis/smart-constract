module ModAddr::FooTaskStore4 {
  ...
  const ETASK_STATE_NOT_MATCHED: u64 = 5;
	...
	struct TaskStore<phantom TaskType> has key {
	...
	public(friend) fun is_initialized<TaskType>(addr: address): bool {
		exists<TaskStore<TaskType>>(addr)
	}

	public(friend) fun initialize<TaskType>(account: &signer) {
	...
	public(friend) fun task_add<TaskType>(task_id: TaskId, meta_data: ASCII::String)
	...
	public(friend) fun task_change_state<TaskType>(task_id: &TaskId, from: u8, to: u8, disable: bool)
  acquires TaskStore, TaskEnabled, TaskDisabled {
	...
	public(friend) fun task_state<TaskType>(task_id: &TaskId): u8 acquires TaskStore {
		// assert contains
		assert_task_contains<TaskType>(task_id, Errors::invalid_argument(ETASK_ID_NOT_FOUND));
  ...
  fun assert_task_contains<TaskType>(task_id: &TaskId, error_code: u64) acquires TaskStore {
    let publisher = FooTaskId4::publisher(task_id);
    let task_table = & borrow_global<TaskStore<TaskType>>(publisher).task_table;
    assert!(
  ...
  fun assert_task_enabled<TaskType>(task_id: &TaskId, error_code: u64): (u64, u64)
  ...
  fun assert_and_change_task_state<TaskType>(task_id: &TaskId, from: u8, to: u8) acquires TaskStore {
		let publisher = FooTaskId4::publisher(task_id);
		let task_table = &mut borrow_global_mut<TaskStore<TaskType>>(publisher).task_table;
  ...
  fun remove_enabled<TaskType>(task_id: &TaskId, task_index_1: u64, task_index_2: u64) acquires TaskEnabled {
    let (publisher, performer) = FooTaskId4::addresses_of(task_id);
  ...
  fun insert_disabled<TaskType>(task_id: &TaskId) acquires TaskDisabled {
    let (publisher, performer) = FooTaskId4::addresses_of(task_id);
  ...
  fun on_task_add<TaskType>(task_id: &TaskId) acquires TaskEnabled {
    let (publisher, performer) = FooTaskId4::addresses_of(task_id);
