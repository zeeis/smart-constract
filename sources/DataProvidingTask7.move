module ModAddr::DataProvidingTask7 {
    use Std::Signer;
    ...
    */
    ...
    public(script) fun initialize<CoinType>(account: &signer) {
        ...
        Task7::initialize<DataProvidingTask>(account);
    }
    ...
    public(script) fun task_publish<CoinType>(
        ...
        let task_id = Task7::task_publish<DataProvidingTask>(receiver, provider_addr, data, meta_bytes);
    ...
    public(script) fun task_cancel<CoinType>(
        ...
        let task_id = Task7::get_task_id_from_publisher(receiver, provider_addr, index);
        Task7::task_change_state<DataProvidingTask>(&task_id, TS_PUBLISHED, TS_CANCELED, true);
    ...
    public(script) fun task_accept_and_submit<CoinType>(
        ...
        let task_id = Task7::get_task_id_from_performer(provider, receiver_addr, index);
        let task_state = if (abandoned) { TS_ABANDONED } else { TS_UNCONFIRM };
        Task7::task_change_state<DataProvidingTask>(&task_id, TS_PUBLISHED, task_state, abandoned);
    ...
    public(script) fun task_accept(
        ...
        let task_id = Task7::get_task_id_from_performer(provider, receiver_addr, index);
        Task7::task_change_state<DataProvidingTask>(&task_id, TS_PUBLISHED, TS_ACCEPTED, false);
    }
    ...
    public(script) fun task_submit<CoinType>(
        ...
        let task_id = Task7::get_task_id_from_performer(provider, receiver_addr, index);
        let task_state = if (abandoned) { TS_ABANDONED } else { TS_UNCONFIRM };
        Task7::task_change_state<DataProvidingTask>(&task_id, TS_ACCEPTED, task_state, abandoned);
        if (abandoned) {
            deposit_back<CoinType>(&task_id);
        }
    }
    ...
    public(script) fun task_confirm<CoinType>(
        ...
        Task7::task_change_state<DataProvidingTask>(&task_id, TS_UNCONFIRM, task_state, true);
        if (failed) {
            deposit_back<CoinType>(&task_id);
        } else {
    ...
    fun deposit_back<CoinType>(task_id: &TaskId) acquires CoinStore {
        let addr = TaskId7::publisher(task_id);
    ...
    fun deposit_to<CoinType>(task_id: &TaskId) acquires CoinStore {
        let (publisher_addr, addr) = TaskId7::addresses_of(task_id);
        let table = &mut borrow_global_mut<CoinStore<CoinType>>(publisher_addr).coin_table;
        let coin = Table::remove<TaskId, Coin<CoinType>>(table, *task_id);
        Coin::deposit<CoinType>(addr, coin);
    }
    ...
    //		let task_id = Task7::get_task_id_from_publisher(&account, addr2, 0);
    //	  assert!(Task7::task_state<DataProvidingTask>(&task_id) == TS_PUBLISHED, 0);
    //
    //	  task_cancel(&account, addr2, 0);
    //	  assert!(Task7::task_state<DataProvidingTask>(&task_id) == TS_CANCELED, 1);
    //
    //	  let DataProvidingTask { amount } = Task7::task_data<DataProvidingTask>(&task_id);
    //	  assert!(amount == 300, 2);
    //	}
}
