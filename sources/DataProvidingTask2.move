module ModAddr::DataProvidingTask2 {
	use Std::Signer;
  ...
  const TS_FAILED: u8 = 6;
	...
	public(script) fun initialize<CoinType> (account: &signer) {
		...
		Task4::initialize<DataProvidingTask>(account);
	}
	...
	public(script) fun task_publish<CoinType>(
		...
		let task_id = Task4::task_publish<DataProvidingTask>(receiver, provider_addr, data, meta_bytes);
	...
	public(script) fun task_cancel<CoinType> (
		...
		let task_id = Task4::get_task_id_from_publisher(receiver, provider_addr, index);
	  Task4::task_change_state<DataProvidingTask>(&task_id, TS_PUBLISHED, TS_CANCELED, true);
    ...
    let task_id = Task4::get_task_id_from_performer(provider, receiver_addr, index);
    Task4::task_change_state<DataProvidingTask>(&task_id, TS_PUBLISHED, TS_UNCONFIRM, false);
  }
	...
	public(script) fun task_accept(
		...
		let task_id = Task4::get_task_id_from_performer(provider, receiver_addr, index);
	  Task4::task_change_state<DataProvidingTask>(&task_id, TS_PUBLISHED, TS_ACCEPTED, false);
	}
	// disabled
	public(script) fun task_submit(
	  ...
	  Task4::task_change_state<DataProvidingTask>(&task_id, TS_ACCEPTED, task_state, abandoned);
	}
	...
	public(script) fun task_confirm<CoinType> (
	  ...
	  Task4::task_change_state<DataProvidingTask>(&task_id, TS_UNCONFIRM, task_state, true);
	  if (failed) {
	    deposit_back<CoinType>(&task_id);
	  } else {
	...
	fun deposit_back<CoinType>(task_id: &TaskId) acquires CoinStore {
		let addr = TaskId4::publisher(task_id);
	...
	fun deposit_to<CoinType>(task_id: &TaskId) acquires CoinStore {
		let (publisher_addr, addr) = TaskId4::addresses_of(task_id);
		let table = &mut borrow_global_mut<CoinStore<CoinType>>(publisher_addr).coin_table;
		let coin = Table::remove<TaskId, Coin<CoinType>>(table, *task_id);
	  Coin::deposit<CoinType>(addr, coin);
	}
...
//		let task_id = Task4::get_task_id_from_publisher(&account, addr2, 0);
//	  assert!(Task4::task_state<DataProvidingTask>(&task_id) == TS_PUBLISHED, 0);
//
//	  task_cancel(&account, addr2, 0);
//	  assert!(Task4::task_state<DataProvidingTask>(&task_id) == TS_CANCELED, 1);
//
//	  let DataProvidingTask { amount } = Task4::task_data<DataProvidingTask>(&task_id);
//	  assert!(amount == 300, 2);
//	}
